...
for frame_idx=1:nimgs
    ...
    if frame_idx == 1
        for blob_idx=1:num_blobs
            ...
            objList = [objList obj];
            drawRectangle(blobs(blob_idx).pos, obj.id, 'yellow');
        end
    else
        for blob_idx=1:num_blobs
            matches = find_matches(blobs(blob_idx), objList, frame_idx, matchingTh);

            num_matches = sum(matches);
            matches_idx = find(matches);
            ...
            elseif num_matches == 1
                objList(matches_idx) = objList(matches_idx).addPos(blobs(blob_idx).pos, frame_idx);
                drawRectangle(blobs(blob_idx).pos, objList(matches_idx).id, 'yellow');
            else
                for i=1:num_matches
                    predicted_pos = objList(matches_idx(i)).getPredictedPos(frame_idx);
                    % TODO adjust added pos based on blob match
                    objList(matches_idx(i)).addPos(blobs(blob_idx).pos, frame_idx);
                    drawRectangle(predicted_pos, objList(matches_idx(i)).id, 'yellow');
                end
            end
        end
    end
    ...
    for i=1:length(objList)
        if frame_idx - objList(i).frameList(end) > 5
            to_remove = [to_remove i];
        end
        % TO REMOVE
%         drawRectangle(objList(i).getPredictedPos(frame_idx), objList(i).id, 'red');
    end
    objList(to_remove) = [];
